...
export class ArtistService {
  ...
  checkArtistExists(label: string): Observable<boolean> {
    return this.http.get<ApiResponseArtist>(`${this.apiUrl}?page=0&size=10&sort=label,asc&label=${encodeURIComponent(label)}`)
      .pipe(
        map(response => response.content.some(artist => artist.label.toLowerCase() === label.toLowerCase())),
        catchError(error => {
          console.error('Erreur lors de la vérification de l\'existence de l\'artiste', error);
          return of(false);
        })
      );
  }
