...
export class EventService {
  ...
  checkEventExists(label: string): Observable<boolean> {
    return this.http
      .get<ApiResponse>(
        `${
          this.apiUrl
        }?page=0&size=10&sort=label,asc&label=${encodeURIComponent(label)}`
      )
      .pipe(
        map((response) =>
          response.content.some(
            (event) => event.label.toLowerCase() === label.toLowerCase()
          )
        ),
        catchError((error) => {
          console.error(
            "Erreur lors de la vérification de l'existence de l'événement",
            error
          );
          return of(false);
        })
